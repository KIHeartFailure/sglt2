...
tab1out$V1 <- ifelse(stri_extract_first_words(tab1out$tmp_varsforfoot) == "shf_ras",
...
colnames(tab1out) <- sanitizeTextFunc(c(
  "Variables", rep(c("Missing (%)", "SGLT2 No", "SGLT2 Yes", "SMD", "p-value"), 2)
))
...
footnote(
  ...
  symbol = c(
    "arb/acei/arni",
    "Included in the multiple imputation model (althought not necessarily imputed if there is no missing data) and models."
  )
)
```
